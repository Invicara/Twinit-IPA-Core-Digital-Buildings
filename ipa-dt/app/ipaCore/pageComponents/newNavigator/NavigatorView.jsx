...
import {newNavStoreRef, setCameraPosition} from "../observability/common/bimViewerCustom";
...
const NavigatorView = (props) => {
    ...
    const filteredBySearchEntityIds = useSelector(Entities.getFilteredBySearchEntityIds)
    ...
    useEffect(()=>{
        ...
        if(viewerMode===NavigatorSource.SEARCH){
            ...
            if(_.isEmpty(allEntitiesBySearch)){
            ...
            } else if(viewerSelectedEntitiesBySearch.length > 0) {
                // if a user selects an entity directly from the model
                ...
                // If user selects an entity directly from the modal, we do not want to open the filter drawer.
                allEntitiesBySearch[0].revitElementIds ? shouldFilterDrawerBeOpened = true: shouldFilterDrawerBeOpened=false;
            } else {
    ...
    const clearSelected = () => {
        setQueryParams({Asset: {}, Space: {}, 'Model Element': {}, System: {}})
    }
    ...
    let cameraPos = {
        "position": {
            "x": -103693.78641462485,
            "y": -105009.79553760799,
            "z": 114373.64927730823
        },
        "target": {
            "x": -4812.134638813485,
            "y": -6128.143761796635,
            "z": 15491.997501496862
        },
        "up": {
        ...
        "width": 248867.0364885479,
        "height": 248867.03648854786,
        "projection": 0,
        "nearLimit": 0.001,
        "className": "Communicator.Camera"
    }
  
    // Runs when the Clear button (EntityClearSearchButton) is clicked in the bottom panel
    useEffect(() => {
        if(viewerRef.current){ 
            newNavStoreRef(viewerRef.current.current, true);
            setCameraPosition(cameraPos)
        }
    }, [queryParams])

    const [reloadToken, setReloadToken] = useState(true)
    // Runs when a user moves to a different page handler
    useEffect(() => {
        if(viewerRef.current){ 
            newNavStoreRef(viewerRef.current.current, true);
            setCameraPosition(cameraPos)
        ...
        <div className='navigator-view'>
            ...
            <div className='navigator-view__panels'>
                <EntitySearchPanels
                    ...
                    setFilteredBySearchEntities={props.setFilteredBySearchEntities}
                    filteredBySearchEntityIds={filteredBySearchEntityIds}
