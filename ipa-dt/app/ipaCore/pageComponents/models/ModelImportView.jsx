...
import ReactTable from '../layouts/Table';
...
import { MISSING_RELATIONS_COLUMNS, VALID_FILE_EXT } from '../utils/constants';
import {
  constructModelFileVersions,
  getCurrentFileNameFromSelect,
  getFileNameAndExtension,
  getSelectedBimpkVersions,
  isCurrentModelAndVersion,
  isTipVersion,
  mapMissingRelations,
  onVersionChecked,
  validateFile,
} from './helpers';
import CustomMaterialPagination from '../layouts/TablePagination';
...
class ModelImportView extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selectedBimpkVersions: [], //the versions of the selected bimpk file
      missingRelations: [],
      entitiesMissingRelations: [],
      missingRelationsColumns: [],
      selectedVersionToImport: null, //the version of the bimpk file which has been selected
      throbberMessage: '',
      ...
      createLinks: false, //whether or not we shoudl do auot-linking to documents,
  ...
  async componentDidMount() {
    ...
    if (this.props.selectedItems.selectedModel && Object.entries(this.props.selectedItems.selectedModel).length) {
      const defaultSelections = bimpkOptions.filter((opt) => opt.label === this.props.selectedItems.selectedModel._name);

      if (defaultSelections.length) defaultSelection = defaultSelections[0];
    }
    ...
    this.setState(
      {
        ...
        isPageLoading: false,
        missingRelationsColumns: MISSING_RELATIONS_COLUMNS,
      },
  ...
  //get all sgpk files in the current project
    ...
    await this.setState({ handler, project, bimpks: modelFiles, selectedBimpkVersions });
  };
  ...
  _getOrchs = async () => {
    ...
    if (datasources) {
      ...
      mappingOrch = _.find(datasources._list, { _userType: 'map_elements_type' });
    }
  ...
  onImport = async () => {
    ...
    if (modelFileExtension === 'bimpk') {
      orch = bimpkOrch;
      orchStepName = ['bimpk_file_extractor', 'default_script_target'];
    ...
    let task
    orchStepName.map(stepName => {
      const newTask = _.find(orch.orchsteps, { _name: stepName })
      if(newTask) {
        task = newTask
        return
      }
  ...
  onRemap = async () => {
    this.setState({ isPageWorking: true, orchRunStatus: null, orchStepRunStatus: [], throbberMessage: 'Remapping Model' });

    const { mappingOrch } = this.state;

    //find the extractor task and get the seqtypeid (all tasks should be the same
    const task = _.find(mappingOrch.orchsteps, { _name: 'Map type map to elements' });
    const seqTypeId = task._compid;
    ...
    this.runOrchestrator(mappingOrch, req, ctx);
  };
      ...
      if (result) {
        if (result.hasOwnProperty('id')) {
          ...
          const interval = setInterval(async () => {
            const { orchStepRunStatus, project, importIntervalId } = this.state;
            ...
            console.log(orchStepRunStatus);
            console.log(errStatus, queuedStatus, runningStatus);
  ...
  onMissingEntitiesDownload = async () => {
    this.setState({ isPageWorking: true });
    await ScriptHelper.executeScript(this.props.handler.config.scripts.download, { missingItems: this.state.entitiesMissingRelations });
    this.setState({ isPageWorking: false });
  };

  _getMissingRelations = async () => {
    const query = { _userType: 'rvt_elements' };

    const modelRels = await IafItemSvc.getRelatedInItem(this.props.selectedItems.selectedModel._id, { query });

    const userItemVersion = await IafItemSvc.getNamedUserItemVersion(modelRels._list[0]._userItemId, modelRels._list[0]._userItemVersionId);

    if (!userItemVersion._userAttributes || !userItemVersion._userAttributes.missingRelationsFromPrevVersion) {
      return;
    }

    this.setState({ missingRelations: userItemVersion._userAttributes.missingRelationsFromPrevVersion });
  };

  onMissingEntitiesRequest = async () => {
    this.setState({ isPageWorking: true });

    const missingElementsJson = await mapMissingRelations(this.state.missingRelations);

    const missingElements = await ScriptHelper.executeScript(this.props.handler.config.scripts.findMissingItems, {
      missingItems: missingElementsJson.flat(),
    });

    this.setState({ entitiesMissingRelations: missingElements, isPageWorking: false });
  ...
  render() {
    ...
    const columns = [
      {
        ...
        id: 'status',
        cell: (row) => {
          // console.log("unicorn row", row)
          return isCurrentModelAndVersion(row, this.state.selectedBimpk, this.props.selectedItems.selectedModel) ? (
            <div>
              <i title="Current Version" style={{ fontSize: '21px', color: 'green' }} className="icon ion-arrow-right-a"></i>
            </div>
          ) : isTipVersion(row, this.state.bimpks, this.state.selectedBimpk) ? (
            <div>
              <input
                type="checkbox"
                checked={row.checked}
                onChange={() =>
                  onVersionChecked(row, this.state.bimpks, (bimpks, checkedVer) => {
                    // console.log("unicorn bimpks, checkedVer, selectedBimpk", bimpks, checkedVer, this.state.selectedBimpk)

                    this.setState({ bimpks, selectedVersionToImport: checkedVer, selectedBimpkVersions: [...bimpks[0].versions] })
                  })
                }
      ...
      {
        name: 'Created on',
        selector: (row) => row.displayCreateDate,
        headerStyle: {
          textAlign: 'left',
        },
        id: 'createdon',
    ...
    const statusArr = ["QUEUED" , "RUNNING"]
        ...
        {!isPageLoading && (
          <div className="tableContainer" style={{ width: '50%', margin: 'auto', paddingTop: '20px' }}>
            ...
            <div style={{ margin: 'auto' }}>
              ...
              <ReactTable
                className="-striped"
                noDataText="No File Version to Import"
                data={selectedBimpkVersions}
                columns={columns}
                defaultPageSize={10}
                // showPagination={selectedBimpkVersions.length > 10}
                pagination
                pageSizeOptions={[10, 20, 25, 50, 100]}
                minRows={1}
                conditionalRowStyles={[{
                  when: (row) => isCurrentModelAndVersion(row, selectedBimpk, selectedItems.selectedModel),
              ...
              <div style={{ textAlign: 'right', marginTop: '15px', display: 'flex', justifyContent: 'center' }}>
                ...
                <GenericMatButton
                  ...
                  disabled={(!bimpkOrch && !sgpkOrch) || isPageLoading || isPageWorking}
                ...
                <GenericMatButton
                  ...
                  disabled={!mappingOrch || isPageLoading || isPageWorking}
                  styles={{ marginRight: missingRelations.length ? '15px' : 0 }}
                >
                  Remap Model
                </GenericMatButton>
                {missingRelations.length > 0 && (
                  <GenericMatButton
                    ...
                    disabled={isPageLoading || isPageWorking}
                    styles={{ marginRight: '15px' }}
                  >
              ...
              {!isPageLoading && entitiesMissingRelations.length > 0 && (
                <div style={{ marginTop: '15px' }}>
                  <h4>Entities Missing From The Model</h4>
                  <ReactTable
                    className="-striped"
                    noDataText="No elements are missing from the model"
                    data={entitiesMissingRelations}
                    columns={missingRelationsColumns}
                    defaultPageSize={10}
                    showPagination={entitiesMissingRelations.length > 10}
                    showPaginationBottom={true}
                    pageSizeOptions={[10, 20, 25, 50, 100]}
                    minRows={1}
                  />
                  <div style={{ textAlign: 'right', marginTop: '15px', display: 'flex', justifyContent: 'flex-end' }}>
                    <GenericMatButton
              ...
              {!!orchRunStatus && (
                <div>
                  <hr />
                  <div>
                  {_.includes(statusArr, orchRunStatus[0]._status) && isPageWorking && (
                    <div>
                  ...
                  {orchRunStatus[0]._status === "COMPLETED" && !isPageWorking && (<h3>Import Complete</h3>)}
                  {orchRunStatus[0]._status === "ERROR" && (
                    <div>
                      <h3>Import Incomplete</h3> 
                      <span style={{ color: 'red' }}>{orchRunStatus[0].orchrunsteps[0]._statusmsg}</span>
                    </div>
                  )}
