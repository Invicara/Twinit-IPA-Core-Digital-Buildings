...
const AssetsProblemTable = (props) => {
  ...
  const getFileList = (files) => {
    ...
    return <ul className="problem-files-message">
      {files.map(probMsg => <li key={probMsg}>{probMsg}</li>)}
    </ul>
                    ...
                    {row.problemObject.problems.map((probRow) => (
                      <TableRow key={probRow.problemAttr}>
                        <TableCell width='20%' component="th" scope="row">
                          {Array.isArray(probRow.problemMessage) ? getFileList(probRow.problemMessage) : probRow.problemMessage}
                        </TableCell>
                        <TableCell width='15%'>{probRow.problemAttr}</TableCell>
                        <TableCell>{probRow.problemValue}</TableCell>
                        <TableCell width='15%'>Asset problems cannot be fixed here</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
...
export default AssetsProblemTable