const copyInverseRelationship = async (params, libraries, ctx) => {
    ...
    console.log(JSON.stringify({
      "message": "Fetching Elements from Latest Collection"
    }));
    let model_rel_coll = await IafScriptEngine.getCollectionInComposite(params.compositeitemid, {
    ...
    console.log(JSON.stringify({
      "message": "Fetching Previous Elements Collection"
    }));
    let model_rel_coll_prev = await IafScriptEngine.getCollectionInComposite(params.compositeitemid, {
      _userType: "rvt_elements"
    }, ctx, {
    ...
    console.log(JSON.stringify({
      "message": "Mapping current version with previous version"
    }));
    ...
    console.log(JSON.stringify({
      "message": "Fetching inverse relations"
    }));
    ...
    if (_count > 0) {
      console.log(JSON.stringify({
        "message": `Found ${els_inv_relationships_prev.length} relationships to update`
      }));
    } else {
      console.log(JSON.stringify({
        "message": `Found ${els_inv_relationships_prev.length} relationships to update - Exiting`
      }));
      return true;
    }
    ...
    console.log(JSON.stringify({
      "message": "Compiling relationships to create"
    }));
    relationshipsToCreate.forEach(x => {
      ...
      }), x.elementUserItemVersionId = model_rel_coll_prev._userItemVersionId, x.elementUserItemVersion = model_rel_coll_prev._userItemVersion;
    });
    console.log(JSON.stringify({
      "message": "Identifying missing relationships"
    }));
    let missingRelationShips = relationshipsToCreate.filter(x => x.relatedToItem == undefined).map(obj => {
      let retObj = {
        _relatedFromId: obj._relatedFromId,
        _relatedUserItemVersionId: obj._relatedUserItemVersionId,
        _isInverse: obj._isInverse,
        _relatedToIds: [obj._relatedToIds],
    ...
    relationshipsToCreate = relationshipsToCreate.filter(x => x.relatedToItem != undefined).map(obj => {
      let retObj = {
    ...
    if (missingRelationShips.length > 0) {
      console.log(JSON.stringify({
        "message": "Processing Missing Relationships"
      }));
      missingRelationsFile = await IafScriptEngine.uploadJson({
        data: missingRelationShips,
        filename: els_coll._userItemId + "_missingRelsFromPrevVer_" + model_rel_coll_prev._userItemVersion + "_" + params.relatedUserType + ".json",
    ...
    if (missingRelationsFile) {
      ...
      if (els_userAttributes.missingRelationsFromPrevVersion.length > 0) {
        let missingRelationsFromPrevVersion = els_userAttributes.missingRelationsFromPrevVersion.concat(missingRelations);
      }
      console.log(JSON.stringify({
        "message": "Finally Updating versions"
      }));
...
async function bimpkPostUpload(_params, libraries, ctx) {
const {
    PlatformApi,
    IafScriptEngine
} = libraries;
...
debugger;
let collection = await IafScriptEngine.getCollections({
    _userType: _params.actualParams.relatedUserType,
...
console.log(JSON.stringify({
    "message": "model -> " + JSON.stringify(bim_model)
}));
if (bim_model._versions.length > 1 && collection._list.length > 0) {
