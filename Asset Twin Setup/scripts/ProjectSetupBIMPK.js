...
async function validate(){
	...
	if(checkImportListAccess('ModelImport')){
		let isModelPathExist = LIB.wbJSON.Path[1][1] ? LIB.wbJSON.Path[1][1] : ''
		statusArr['Model Import'] = fs.existsSync(isModelPathExist) ? 'Available' : 'Not Available'
	}
	if(checkImportListAccess('BulkFileUpload')){
		 let isFilesExist = getLocalPaths('Files Path')
		statusArr['Bulk File Path'] = fs.existsSync(isFilesExist) ? 'Available' : 'Not Available'
...
async function createOrRecreateBIMPKDatasource() {
	...
	const orderGroupFunctionResult = await new Promise((resolve) =>
		setTimeout(async () => {
			...
			if (checkImportListAccess('ModelImport')) {
				...
				await createOrRecreateBIMPKDatasource()
...
async function mapElementsType(){ 
      
    //const {LIB.IafScriptEngine } = libraries;
    let iaf_ext_current_bim_model = await LIB.IafScriptEngine.getCompositeCollections(
        { query: { "_userType": "bim_model_version",
          "_itemClass": "NamedCompositeItem"}},
    LIB.ctx, { getLatestVersion: true });
    ...
    let model_els_coll = await LIB.IafScriptEngine.getCollectionInComposite(latestModelComposite._userItemId, { _userType: "rvt_elements" }, LIB.ctx)
    console.log("model_els_coll-easyAssetTwin", model_els_coll)

    let model_type_el_coll = await LIB.IafScriptEngine.getCollectionInComposite(latestModelComposite._userItemId, { _userType: "rvt_type_elements" }, LIB.ctx)
    console.log("model_type_el_coll-easyAssetTwin", model_type_el_coll)

    let type_map_collection = await LIB.IafScriptEngine.getCollection(
		{
    ...
    let typeElements =  await LIB.IafScriptEngine
    .getItems({
    ...
    let assetTypeMap =  await LIB.IafScriptEngine
    .getItems({
        collectionDesc: {
    ...
    let elements =  await LIB.IafScriptEngine
    .getItems({
        _userItemId: model_els_coll._userItemId,
        options: { page: {getAllItems: true, _pageSize:1000}}
    },LIB.ctx);
    ...
    await LIB.IafScriptEngine.updateItemsBulk({
        _userItemId: model_els_coll._userItemId,
        items: elements
...
async function mapAssetCollection() { 
	//const { LIB.IafScriptEngine } = libraries;
	console.log("mapAssetCollection Func Start")
	await mapElementsType();
	
	let assetCollection = await LIB.IafScriptEngine
		.getCollection({
	...
	if(assetCollection){
		let assets = await LIB.IafScriptEngine
			.getItems({
...
async function importModelFile() {
	...
	let bimpk = await getExistingModel('bimpk');

	console.log('bimpk', bimpk)

	let getVersions = await IafFileSvc.getFileVersions(bimpk._id, LIB.ctx);
	...
	//run orchestrator
	let result = await LIB.PlatformApi.IafDataSource.runOrchestrator(bimpkOrch.id, orchReq, LIB.ctx);
...
async function isAssetAvailable(findUser){
	...
	if ((findUser[0]["Assets"] == "Yes" && isScriptExists("iaf_entass_allusers")) || (LIB.updateFlag && findUser[0]["Assets"] == "Yes")) {
		...
		Object.assign(CONFIGVARS.updateUserConfigContent,
			...
			console.log(CONFIGVARS.entitySelectConfigAssets.entitySelectConfig.Asset[1],'bimpkScript');
...
async function loadScripts(findUser) {
	...
	if (findUser[0]["BIMPK Upload"] == "Yes" && isScriptExists("iaf_bimpk_post_imp")) {
...
async function isSpacePropsAvailable(){
	...
	if(spaceProps && checkImportListAccess('Spaces')){
		let spaceGroups = {}
		var spaceGroupLen = 0
...
let ProjSetup = {
	...
	async easyAssetTwinSetup(input, libraries, ctx, callback) {
		...
		LIB.proj = proj;
			...
			if(await validate()){
				...
				if(checkImportListAccess('BimType')){
				//This function is used to import bim tyeps
					await typeMapLoader()
					//This function is used to map the asset collection
					await mapAssetCollection()
				}
				...
				//This condition is used to check BulkFileUpload in import list if yes then execute the function 
				if(checkImportListAccess('BulkFileUpload')){
				//This function is used to upload files bulk with using a sheet
					await bulkUploadFileWithAttributes()
				}
				//This condition is used to check AssetFileRelation in import list
				if(checkImportListAccess('AssetFileRelation')){
				//This function is used to create relation between Asset and files
	...
	async updateEasyAssetProject(input, libraries, ctx, callback) {
		...
		//This function used to validate the sheet
			...
			const endTime = new Date().getTime()
			//This Function is used to log the total execution time 
			logScriptExecutionTime(startTime, endTime); 
	}
}
