let entass = {
    ...
    //     return tableDataWithHeader
    // },
    async getSpacesForAsset(input, libraries, ctx, callback) {
        let { PlatformApi } = libraries;
        let iaf_asset_collection = PlatformApi.IafScriptEngine.getVar('iaf_asset_collection')
        let assetDoc_query = {
            query:
            {
                ...
                related: [
                    {
                        relatedDesc: { _relatedUserType: "iaf_ext_space_coll", _isInverse: true },
                        as: "Spaces",
                        options: { page: { getAllItems: true } }
                    }
                ]
        ...
        console.log(IAF_assetdoc_res, 'IAF_assetdoc_res');
        let spacesForAsset = IAF_assetdoc_res._list[0].Spaces._list
        console.log(spacesForAsset,'spacesForAsset');
        return spacesForAsset
    },
