let entspa = {
    ...
    async spacesToAssets(input, libraries, ctx) {
        ...
        let assetRoomQueries = input.entityInfo.selectedEntities.map(space => {
            return {
                query: { "properties.AssetRel.val": space.properties["SpaceRel"].val  },
    ...
    async getAssetsForSpace(input, libraries, ctx) {
        ...
        let IAF_spaceAsset_res = await PlatformApi.IafScriptEngine.findWithRelated(spaceAsset_query, ctx)
        console.log('IAF_spaceAsset_res', IAF_spaceAsset_res)
        return IAF_spaceAsset_res._list[0].assets._list
    },
